# Compares Puid and SecureRandom.uuid() for UUID v4 generation
# Includes performance benchmarks and ERE, ETE metrics
#
# Usage:
#   MIX_ENV=test mix run bench/puid_secure_random_uuid.exs

Code.require_file("benchmark_helper.exs", __DIR__)

defmodule Bench.PuidSecureRandomUuid do
  @trials 500_000
  @entropy_bits 122
  @random_bytes 16

  def run do
    defmodule PuidHex do
      use Puid, bits: 122, chars: :hex
    end

    secure_random_id = SecureRandom.uuid()
    puid_id = PuidHex.generate()

    IO.puts("\n# SecureRandom.uuid() vs Puid Comparison")
    IO.puts("")

    IO.puts("## Charset: hex")
    IO.puts("- Size: 16 characters (0-9, a-f)")
    IO.puts("- SecureRandom.uuid format: 8-4-4-4-12 (with dashes)")
    IO.puts("- Puid format: continuous hex string")
    IO.puts("")

    IO.puts("## Example IDs")
    IO.puts("- SecureRandom: `#{secure_random_id}`")
    IO.puts("- Puid:         `#{puid_id}`")
    IO.puts("")

    sr_chars = String.length(secure_random_id)
    puid_chars = String.length(puid_id)

    sr_ere = Float.round(@entropy_bits / (sr_chars * 8), 2)
    puid_ere = Float.round(@entropy_bits / (puid_chars * 8), 2)

    sr_ete = Float.round(@entropy_bits / (@random_bytes * 8), 2)
    puid_ete = 1.0

    IO.puts("## Performance Benchmark")
    IO.puts("  Generate #{@trials} random IDs with #{@entropy_bits} bits of entropy")

    secure_random = fn ->
      for _ <- 1..@trials, do: SecureRandom.uuid()
    end

    puid = fn ->
      for _ <- 1..@trials, do: PuidHex.generate()
    end

    IO.puts("")
    sr_time = BenchmarkHelper.time(secure_random, "    SecureRandom.uuid() (CSPRNG)")
    puid_time = BenchmarkHelper.time(puid, "    Puid                (CSPRNG)")

    IO.puts("")
    IO.puts("  Performance: #{BenchmarkHelper.format_performance(sr_time, puid_time)}")
    IO.puts("")

    BenchmarkHelper.metrics_table(
      "SecureRandom",
      sr_chars,
      sr_ere,
      sr_ete,
      puid_chars,
      puid_ere,
      puid_ete,
      @trials,
      sr_time,
      puid_time
    )

    IO.puts("\n## Summary")
    IO.puts("- SecureRandom: #{sr_chars} characters (includes 4 dashes)")
    IO.puts("- Puid: #{puid_chars} characters (no dashes)")
    IO.puts("- Puid's ERE is #{Float.round(puid_ere / sr_ere, 2)}x better")
    IO.puts("- Puid generates #{Float.round((1 - puid_chars / sr_chars) * 100, 0)}% shorter IDs")
    IO.puts("- UUID format wastes 32 bits on dashes that carry no entropy")
  end
end

Bench.PuidSecureRandomUuid.run()

# Tests the theoretical :bit_shift ETE calculation by running trials and measuring actual bit consumption
#
# Usage:
#   mix run bench/test_bit_shift_ete.exs [charset] [trials]
#
# Example:
#   mix run bench/test_bit_shift_ete.exs decimal 1000000
#   mix run bench/test_bit_shift_ete.exs alphanum_lower

defmodule ETETestBitShift do
  ...
  def run(args) do
    ...
    metrics = Puid.Chars.metrics(charset, :bit_shift)
    bit_shift_ete = metrics.ete
    ...
    if bit_shift_ete != naive_ete do
      improvement = (bit_shift_ete / naive_ete - 1) * 100
      IO.puts("bit_shift improvement: #{Float.round(improvement, 1)}%")
    end
    ...
    rand_module_name = String.to_atom("ETETestBitShiftRand_#{charset}")
    ...
    module_name = String.to_atom("ETETestBitShift_#{charset}")
    ...
    IO.puts(
      "Difference from bit_shift: #{Float.round((empirical_ete - bit_shift_ete) * 100, 2)}%"
    )

    IO.puts("")

    if abs(empirical_ete - bit_shift_ete) < 0.01 do
      IO.puts("✓ Empirical and calculated bit_shift ETE match closely!")
    else
    ...
    if bit_shift_ete != naive_ete do
      ...
      IO.puts("")
      IO.puts("bit_shift vs Naive Comparison:")
      IO.puts("-" |> String.duplicate(40))
  ...
  defp parse_args(_) do
    IO.puts("Usage: mix run bench/test_bit_shift_ete.exs [charset] [trials]")
    System.halt(1)
  end
...
ETETestBitShift.run(System.argv())
